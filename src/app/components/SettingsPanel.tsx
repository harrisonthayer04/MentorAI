    ...
    <>
      ...
      >
        ...
        <div className="sticky top-0 z-10 bg-[var(--color-surface)]/95 backdrop-blur-sm border-b border-[var(--color-border)] px-6 py-4 flex items-center justify-between">
          <h1 className="text-lg font-display font-semibold text-[var(--color-text)]">Settings</h1>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-3 border-b border-[var(--color-border)]">
          <div className="flex gap-1 bg-[var(--color-surface-elevated)]/50 rounded-lg p-1">
            {[
          ...
          {activeTab === "general" && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  <h2 className="text-sm font-semibold text-[var(--color-text)] uppercase tracking-wide">Accent Color</h2>
                  <p className="text-xs text-[var(--color-text-muted)] mt-1">Choose your preferred accent color</p>
                </div>
                  ...
                  {PRESET_COLORS.map((preset) => (
                    <button
                      ...
                      style={{
                        ...
                        boxShadow: accentColor === preset.color ? `0 0 0 2px var(--color-surface), 0 0 0 4px ${preset.color}` : "none",
                      }}
                      title={preset.name}
                    >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    placeholder="#6366f1"
                    className="flex-1 rounded-lg bg-[var(--color-surface-elevated)] border border-[var(--color-border)] px-3 py-2 text-sm font-mono uppercase text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-[var(--color-ring)]/50"
                  />
                  <button
                    onClick={() => handleColorChange(DEFAULT_ACCENT_COLOR)}
                    className="px-3 py-2 rounded-lg bg-[var(--color-surface-elevated)] text-[var(--color-text-secondary)] text-sm hover:bg-[var(--color-surface-hover)] hover:text-[var(--color-text)] transition-colors"
                  >
                    Reset
                  </button>
                </div>
              ...
              <div className="pt-6 border-t border-[var(--color-border)] space-y-4">
                <div>
                  <h2 className="text-sm font-semibold text-[var(--color-text)] uppercase tracking-wide">Developer</h2>
                  <p className="text-xs text-[var(--color-text-muted)] mt-1">Advanced options for debugging</p>
                </div>

                <div className="flex items-center justify-between p-4 rounded-xl bg-[var(--color-surface-elevated)]/50 border border-[var(--color-border)]">
                  <div>
                    <div className="text-sm font-medium text-[var(--color-text)]">Debug logging</div>
                    <div className="text-xs text-[var(--color-text-muted)] mt-0.5">Enable verbose logging for chat transcripts</div>
                  </div>
                  <button
                    role="switch"
                    ...
                    style={{ backgroundColor: debugModeEnabled ? 'var(--color-brand)' : 'var(--color-surface-hover)' }}
                  >
                    <span
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full rounded-lg bg-[var(--color-surface-elevated)] border border-[var(--color-border)] px-3 py-2 text-sm text-[var(--color-text)] placeholder-[var(--color-text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--color-ring)]/50"
                />
                <textarea
                  value={newContent}
                  ...
                  rows={3}
                  className="w-full rounded-lg bg-[var(--color-surface-elevated)] border border-[var(--color-border)] px-3 py-2 text-sm text-[var(--color-text)] placeholder-[var(--color-text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--color-ring)]/50 resize-none"
                />
                <button
                  onClick={createMemory}
                  disabled={!newContent.trim()}
                  className="w-full rounded-lg text-white text-sm font-semibold py-2.5 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              <div className="pt-6 border-t border-[var(--color-border)]">
                {loading ? (
                  ...
                  <div className="text-center py-8">
                    <div className="w-12 h-12 rounded-xl bg-[var(--color-surface-elevated)] mx-auto mb-3 flex items-center justify-center">
                    ...
                    <p className="text-sm text-[var(--color-text-muted)]">No memories yet</p>
                    <p className="text-xs text-[var(--color-text-muted)] mt-1">Add memories to help personalize your experience</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {memories.map((m) => (
                      <div key={m.id} className="rounded-xl bg-[var(--color-surface-elevated)]/50 border border-[var(--color-border)] p-4">
                        <div className="flex items-start gap-2 mb-2">
                          <input
                            defaultValue={m.title || ""}
                            placeholder="Title"
                            onBlur={(e) => updateMemory(m.id, { title: e.target.value })}
                            className="flex-1 bg-transparent border-none text-sm font-medium text-[var(--color-text)] focus:outline-none placeholder-[var(--color-text-muted)]"
                          />
                          <button
                        ...
                        <textarea
                          ...
                          rows={2}
                          className="w-full bg-transparent border-none text-sm text-[var(--color-text-secondary)] focus:outline-none resize-none"
                        />
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-6">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-red-500/10 border border-red-500/30 text-red-500 text-sm font-medium hover:bg-red-500/20 transition-colors disabled:opacity-50"
                >
